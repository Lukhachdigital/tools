...
const Lightbox = ({ imageUrl, onClose }: { imageUrl: string; onClose: () => void }): React.ReactElement => {
    ...
    return (
      React.createElement("div",
        ...
        React.createElement("div",
          innerDivProps,
          React.createElement("img", { src: imageUrl, alt: "Generated Image", className: "w-full h-full object-contain rounded-lg shadow-2xl" }),
...
const ContextCard = ({ context, contextIndex, onGenerateImage, imageData, onImageClick }: {
    context: ContextItem;
    contextIndex: number;
    onGenerateImage: (index: number, prompt: string, ratio: '16:9' | '9:16') => void;
    imageData: { imageUrl?: string; isGenerating?: boolean; error?: string; };
  ...
  return (
    React.createElement("div", { className: "bg-gray-900/50 p-4 rounded-lg border border-gray-700" },
       React.createElement("div", { className: "flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-3" },
         React.createElement("div", { className: "flex items-center gap-3" },
           React.createElement("h4", { className: "text-lg font-bold text-gray-100" }, context.name),
           React.createElement("span", {
               className: "text-xs font-semibold px-2.5 py-1 rounded-full bg-purple-500/20 text-purple-300"
           }, "Bối cảnh")
         ),
         React.createElement("div", { className: "flex items-center gap-2 flex-shrink-0" },
            React.createElement("button", { onClick: () => setAspectRatio('16:9'), className: `px-3 py-1.5 text-xs rounded-md font-semibold transition ${aspectRatio === '16:9' ? 'bg-blue-600 text-white' : 'bg-gray-600 hover:bg-gray-500 text-gray-300'}` }, "Ngang"),
            React.createElement("button", { onClick: () => setAspectRatio('9:16'), className: `px-3 py-1.5 text-xs rounded-md font-semibold transition ${aspectRatio === '9:16' ? 'bg-blue-600 text-white' : 'bg-gray-600 hover:bg-gray-500 text-gray-300'}` }, "Dọc"),
            React.createElement("button", {
                onClick: () => onGenerateImage(contextIndex, context.whiskPrompt, aspectRatio),
                disabled: isGenerating,
                className: "bg-green-600 hover:bg-green-700 text-white text-sm font-bold py-1.5 px-4 rounded-md disabled:opacity-50 disabled:cursor-wait transition"
            }, isGenerating ? "Đang tạo..." : "Tạo ảnh")
         )
       ...
       React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-4" },
         React.createElement("div", null,
           React.createElement("p", { className: "text-gray-300 text-sm mb-4" }, context.description),
           React.createElement("div", null,
             React.createElement("p", { className: "font-semibold text-sm text-purple-300 mb-2" }, "Prompt tạo ảnh bối cảnh:"),
             React.createElement("div", { className: "relative bg-gray-700 p-3 rounded-md border border-gray-600" },
               React.createElement("p", { className: "text-gray-200 text-xs break-words pr-24" }, context.whiskPrompt),
               React.createElement("button", {
         ...
         React.createElement("div", { className: `w-full ${imageContainerAspectRatio} bg-gray-800/50 rounded-lg flex items-center justify-center overflow-hidden border border-gray-700 transition-all` },
            ...
            imageUrl && !isGenerating && !error && (
                React.createElement("img", {
                    src: imageUrl,
                    alt: `Generated image of ${context.name}`,
                    className: "w-full h-full object-cover rounded-md cursor-pointer hover:scale-105 transition-transform duration-300",
                    onClick: () => onImageClick(imageUrl)
                })
            ),
...
const VietKichBanApp = ({ geminiApiKey, openaiApiKey, selectedAIModel }: { geminiApiKey: string, openaiApiKey: string, selectedAIModel: string }): React.ReactElement => {
  ...
  const [isContextConsistent, setIsContextConsistent] = useState(false); // New Toggle State
  ...
  const [contextImages, setContextImages] = useState<{ [key: number]: { imageUrl?: string; isGenerating?: boolean; error?: string; } }>({});
  ...
  const generateScript = useCallback(async (
    ...
    // Logic for Context Consistency
    let contextTaskInstruction = "";
    let contextConsistencyRule = "";
    let contextSchemaProperty = {};
    
    if (isConsistent) {
        contextTaskInstruction = `
**Task 1.5: Context Development (Consistent Mode Active)**
- Identify all key locations/settings (Contexts) that appear in the story.
- If a setting appears 2 or more times, you MUST create a defined Context for it.
- For each context, provide:
    1. **name**: A unique name for the context (e.g., "Context 1", "Context 2", "Living Room", "Ancient Temple").
    2. **description**: A detailed description in VIETNAMESE.
    3. **whiskPrompt**: A detailed, cinematic prompt in ENGLISH for generating this background image. It should NOT have characters in it, just the environment.
`;
        contextConsistencyRule = `
    3.  **CONSISTENT CONTEXT NAMING:** Since "Context Consistency" is ON, for every video prompt that takes place in one of your defined contexts (from Task 1.5), you MUST explicitly start the description of the setting with the context name. Example: "Background: Context 1, a dark cave..." or "Setting: Living Room, a cozy space...".
`;
        contextSchemaProperty = {
            contextList: {
                type: window.GenAIType.ARRAY,
                description: "A list of context/location objects for consistent backgrounds.",
                items: {
                    type: window.GenAIType.OBJECT,
                    properties: {
                        name: { type: window.GenAIType.STRING, description: "The context name (e.g. Context 1)." },
                        description: { type: window.GenAIType.STRING, description: "Vietnamese description of the setting." },
                        whiskPrompt: { type: window.GenAIType.STRING, description: "English prompt for generating the background image." }
                    },
...
Based on the user's idea and your strict adherence to the cinematic style, you must perform tasks and return the result as a single JSON object.
...
- Context Consistency: ${isConsistent ? "ON" : "OFF"}
`;
    ...
    try {
        if (selectedAIModel === 'gemini') {
            ...
            const schema = {
                ...
                required: ["characterList", "prompts"] // contextList is optional in basic requirement, but good to enforce if present
            };
            ...
            let schemaDesc = `{ "characterList": [ ... ], "prompts": [ ... ] }`;
            if (isConsistent) {
                schemaDesc = `{ "characterList": [ ... ], "contextList": [ ... ], "prompts": [ ... ] }`;
            }

            const systemPrompt = `${commonPrompt}\n\nGenerate a JSON object that strictly adheres to the following structure: ${schemaDesc}`;
  ...
  // New function to generate Context Image
  const handleGenerateContextImage = useCallback(async (contextIndex: number, prompt: string, aspectRatio: '16:9' | '9:16') => {
    if (!geminiApiKey) {
      setError("API Key Gemini chưa được cấu hình.");
      return;
    }
    ...
    setContextImages(prev => ({
        ...prev,
        [contextIndex]: { isGenerating: true, error: undefined, imageUrl: prev[contextIndex]?.imageUrl }
    }));
    ...
    try {
        ...
        let finalPrompt = prompt;
        if (selectedCinematicStyle !== 'Hoạt hình') {
          finalPrompt = `ultra photorealistic, realistic photograph, cinematic shot. ${prompt}. The final image must be absolutely realistic, not animated, not 3D, not a cartoon, not fantasy. No characters in the image.`;
        }
        ...
        setContextImages(prev => ({
            ...prev,
            [contextIndex]: { isGenerating: false, imageUrl: imageUrl }
        }));

    } catch (err: any) {
        console.error("Context Image generation error:", err);
        const errorMessage = err.message || "Lỗi không xác định khi tạo ảnh bối cảnh.";
        setContextImages(prev => ({
            ...prev,
            [contextIndex]: { isGenerating: false, error: errorMessage }
        }));
    }
  }, [geminiApiKey, selectedCinematicStyle]);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const content = await generateScript(videoIdea, durationNum, selectedCinematicStyle, mainChars, supChars, isContextConsistent);
  ...
  }, [videoIdea, duration, selectedCinematicStyle, generateScript, numMainCharacters, numSupportingCharacters, isContextConsistent]);
          ...
          React.createElement("div", { className: "flex flex-col md:flex-row md:gap-8 lg:gap-12" },
            ...
            React.createElement("div", { className: "md:w-2/5 lg:w-1/3 mb-8 md:mb-0" },
              React.createElement("form", { onSubmit: handleSubmit, className: "bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700 sticky top-8" },
                ...
                React.createElement("div", { className: "mb-6" },
                   React.createElement("label", { className: "block text-gray-200 text-sm font-bold mb-2" }, "Tùy chọn bối cảnh:"),
                   React.createElement("div", { className: "flex gap-2" },
                       React.createElement("button", {
                           type: "button",
                           onClick: () => setIsContextConsistent(false),
                           className: `flex-1 px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${!isContextConsistent ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-700 text-gray-300 hover:bg-gray-600 border border-gray-600'}`
                       }, "Không nhất quán"),
                       React.createElement("button", {
                           type: "button",
                           onClick: () => setIsContextConsistent(true),
                           className: `flex-1 px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${isContextConsistent ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-700 text-gray-300 hover:bg-gray-600 border border-gray-600'}`
                       }, "Nhất quán")
                   )
                ),
            ...
            React.createElement("div", { className: "md:w-3/5 lg:w-2/3 md:h-[calc(100vh-150px)] md:overflow-y-auto custom-scrollbar md:pr-4" },
              ...
              generatedContent && (
                React.createElement("div", null,
                  ...
                  generatedContent.contextList && generatedContent.contextList.length > 0 && (
                      React.createElement("div", { className: "bg-gray-800 rounded-lg shadow-lg p-6 mb-8 border border-gray-700" },
                        React.createElement("h3", { className: "text-xl font-bold text-purple-400 mb-4" }, "Danh sách Bối cảnh (Nhất quán)"),
                        React.createElement("div", { className: "space-y-6" },
                            generatedContent.contextList.map((ctx, index) => React.createElement(ContextCard, {
                                key: index,
                                context: ctx,
                                contextIndex: index,
                                onGenerateImage: handleGenerateContextImage,
                                imageData: contextImages[index] || {},
                                onImageClick: (url) => setLightboxImage(url)
                            }))
                        )
                      )
